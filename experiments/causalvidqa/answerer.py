...
class Answerer():
    ...
    def init_update(self) -> Tuple[List[float], List[Tuple[int, PIL.Image.Image]], List[str]]:
        ...
        gotos = [c * self.video.length_secs for c in np.linspace(0, 0.99, 5)]
    ...
    def forward(self, video: VideoObj) -> int:
        ...
        log_all([prompt_log, output_log], f"=== reasons: {self.video.reasons} ===")
        ...
        for i in range(self.max_tries): 
            ...
            if self.video.reasons:
                output_dict = self.evaluator.forward(self.video.question, self.video.choices, self.video, plan, reasons=self.video.reasons, with_reason=True)
                final_choice = output_dict["final_answer"]
                final_reason = output_dict["final_reason"]
                output_log.info(f"FINAL CHOICE: {final_choice}")
                output_log.info(f"FINAL REASON: {final_reason}")
            else:
                output_dict = self.evaluator.forward(self.video.question, self.video.choices, self.video, plan)
                final_choice = output_dict["final_answer"]
                output_log.info(f"FINAL CHOICE: {final_choice}")
            ...
            if self.video.reasons and final_reason is not None:
                if final_choice is not None:
                    ...
                    return final_choice, final_reason
            if "final_reason" not in output_dict:
                if final_choice is not None:
                    for image in self.video.images:
                        del image
                    self.video.images.clear()
        ...
        if self.video.reasons:
            output_dict = self.evaluator.forward(self.video.question, self.video.choices, self.video, plan, final_choice=True, reasons=self.video.reasons, with_reason=True)
            final_choice = output_dict["final_answer"]
            final_reason = output_dict["final_reason"]
            output_log.info(f"FINAL CHOICE: {final_choice}")
            output_log.info(f"FINAL REASON: {final_reason}")
            ...
            output_dict = self.evaluator.forward(self.video.question, self.video.choices, self.video, plan, final_choice=True)
            final_choice = output_dict["final_answer"]
            output_log.info(f"FINAL CHOICE: {final_choice}")
            return final_choice, -1
...
class Extractor(Answerer):
    ...
    def parse_answers(self, answers: List[str]) -> List[List[str]]:
        ...
        for answer in answers:
            try:
                parsed_json = ast.literal_eval(answer) # supposed to be list
                parsed_answer = parsed_json['questions']
...
class Summarizer(Answerer):
    ...
    def get_summary(self, info: Dict[str, List[str]], video: VideoObj) -> str:
        ...
        prompt_log.info(f"SUMMARIZER PROMPT: {prompt}")
        return prompt
...
class Evaluator(Answerer):
    ...
    def evaluate_info(self, question: str, choices: List[str], video: VideoObj, plan: str, reasons: list = [], with_reason=False) -> str:
        if with_reason:
            prompt_path = config["evaluator"]["evaluator_reason_prompt"]
        else:
            prompt_path = config["evaluator"]["evaluator_prompt"]
            
        
        if with_reason:
            prompt =  self.answerer.construct_prompt(question, str(choices), video.info, prompt_path).replace("INSERT_LENGTH", str(video.length_secs)).replace("INSERT_PLAN_HERE", plan).replace("INSERT_REASONS_HERE", str(reasons))
        else:
            prompt =  self.answerer.construct_prompt(question, str(choices), video.info, prompt_path).replace("INSERT_LENGTH", str(video.length_secs)).replace("INSERT_PLAN_HERE", plan)
    ...
    def final_select(self, question: str, choices: list, video_info: dict, reasons: list = [], with_reason=False):
        if with_reason:
            prompt_path = config["evaluator"]["final_select_reason"]
        else:
            prompt_path = config["evaluator"]["final_select"]
            
        if with_reason:
            prompt = self.answerer.construct_prompt(question, str(choices), video_info, prompt_path).replace("INSERT_LENGTH", str(self.answerer.video.length_secs)).replace("INSERT_REASONS_HERE", str(reasons))
        else:
            prompt = self.answerer.construct_prompt(question, str(choices), video_info, prompt_path).replace("INSERT_LENGTH", str(self.answerer.video.length_secs))
    ...
    def parse_output(self, answer, with_reason=False):
        try:
            regex_pattern = r'\{[^{}]*\}'
            matches = re.findall(regex_pattern, answer)
            answer_dict = ast.literal_eval(matches[0])
            explanation = answer.split("{")[0].strip()
            final_answer = answer_dict["final_answer"]
            
            if with_reason:
                final_reason = answer_dict["final_reason"]
            
            if final_answer not in [None, "None"]:
                final_answer = int(final_answer)
            if with_reason and final_reason not in [None, "None"]:
                final_reason = int(final_reason)
                
            if with_reason:
                return {"final_answer": final_answer, "final_reason": final_reason, "explanation": explanation}
            else:
                return {"final_answer": final_answer, "explanation": explanation}
        except Exception:
            print("ERROR in Evaluator `parse_output`.")
            print(answer)
            print(traceback.format_exc())
    
    def forward(self, question: str, choices: List[str], video: VideoObj, plan: str, final_choice: bool = False, reasons=[], with_reason=False):
        for _ in range(config["max_retries"]):
            try:
                if not final_choice:
                    output = self.evaluate_info(question, choices, video, plan, reasons, with_reason)
                else:
                    output = self.final_select(question, choices, video.info, reasons, with_reason)

                output_dict = self.parse_output(output, with_reason=with_reason)
                
                final_output = output_dict['final_answer']
                if with_reason:
                    final_reason = output_dict['final_reason']
            ...
            if final_choice:
                ...
                if with_reason:
                    return {"final_answer": final_output, "final_reason": final_reason}
                else:
                    return {"final_answer": final_output, "final_reason": -1}
